...
module Godot.Core.Shape
       (Godot.Core.Shape.get_debug_mesh, Godot.Core.Shape.get_margin,
        Godot.Core.Shape.set_margin)
       where
import Data.Coerce
import Foreign.C
...
{-# NOINLINE bindShape_get_debug_mesh #-}

-- | Returns the @ArrayMesh@ used to draw the debug collision for this @Shape@.
bindShape_get_debug_mesh :: MethodBind
bindShape_get_debug_mesh
  = unsafePerformIO $
      withCString "Shape" $
        \ clsNamePtr ->
          withCString "get_debug_mesh" $
            \ methodNamePtr ->
              godot_method_bind_get_method clsNamePtr methodNamePtr

-- | Returns the @ArrayMesh@ used to draw the debug collision for this @Shape@.
get_debug_mesh ::
                 (Shape :< cls, Object :< cls) => cls -> IO ArrayMesh
get_debug_mesh cls
  = withVariantArray []
      (\ (arrPtr, len) ->
         godot_method_bind_call bindShape_get_debug_mesh (upcast cls) arrPtr
           len
           >>= \ (err, var) -> throwIfErr err >> fromGodotVariant var)

instance NodeMethod Shape "get_debug_mesh" '[] (IO ArrayMesh) where
        nodeMethod = Godot.Core.Shape.get_debug_mesh
