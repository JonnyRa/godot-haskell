...
module Godot.Core.BakedLightmap
       (Godot.Core.BakedLightmap._BAKE_QUALITY_LOW,
        ...
        Godot.Core.BakedLightmap.get_bounce_indirect_energy,
        ...
        Godot.Core.BakedLightmap.set_bounce_indirect_energy,
...
instance NodeProperty BakedLightmap "bounce_indirect_energy" Float
           'False
         where
        nodeProperty
          = (get_bounce_indirect_energy,
             wrapDroppingSetter set_bounce_indirect_energy, Nothing)
...
{-# NOINLINE bindBakedLightmap_get_bounce_indirect_energy #-}
...
--   			__Note:__ @bounce_indirect_energy@ only has an effect if @bounces@ is set to a value greater than or equal to @1@.
bindBakedLightmap_get_bounce_indirect_energy :: MethodBind
bindBakedLightmap_get_bounce_indirect_energy
  = unsafePerformIO $
      withCString "BakedLightmap" $
        \ clsNamePtr ->
          withCString "get_bounce_indirect_energy" $
            \ methodNamePtr ->
              godot_method_bind_get_method clsNamePtr methodNamePtr
...
get_bounce_indirect_energy ::
                             (BakedLightmap :< cls, Object :< cls) => cls -> IO Float
get_bounce_indirect_energy cls
  = withVariantArray []
      (\ (arrPtr, len) ->
         godot_method_bind_call bindBakedLightmap_get_bounce_indirect_energy
           (upcast cls)
           arrPtr
           len
           >>=
...
instance NodeMethod BakedLightmap "get_bounce_indirect_energy" '[]
           (IO Float)
         where
        nodeMethod = Godot.Core.BakedLightmap.get_bounce_indirect_energy
...
{-# NOINLINE bindBakedLightmap_set_bounce_indirect_energy #-}

-- | The energy multiplier for each bounce. Higher values will make indirect lighting brighter. A value of @1.0@ represents physically accurate behavior, but higher values can be used to make indirect lighting propagate more visibly when using a low number of bounces. This can be used to speed up bake times by lowering the number of @bounces@ then increasing @bounce_indirect_energy@. Unlike @BakedLightmapData.energy@, this property does not affect direct lighting emitted by light nodes, emissive materials and the environment.
--   			__Note:__ @bounce_indirect_energy@ only has an effect if @bounces@ is set to a value greater than or equal to @1@.
bindBakedLightmap_set_bounce_indirect_energy :: MethodBind
bindBakedLightmap_set_bounce_indirect_energy
  = unsafePerformIO $
      withCString "BakedLightmap" $
        \ clsNamePtr ->
          withCString "set_bounce_indirect_energy" $
            \ methodNamePtr ->
              godot_method_bind_get_method clsNamePtr methodNamePtr
...
set_bounce_indirect_energy ::
                             (BakedLightmap :< cls, Object :< cls) => cls -> Float -> IO ()
set_bounce_indirect_energy cls arg1
  = withVariantArray [toVariant arg1]
      (\ (arrPtr, len) ->
         godot_method_bind_call bindBakedLightmap_set_bounce_indirect_energy
           (upcast cls)
           arrPtr
           len
           >>=
...
instance NodeMethod BakedLightmap "set_bounce_indirect_energy"
           '[Float]
           (IO ())
         where
        nodeMethod = Godot.Core.BakedLightmap.set_bounce_indirect_energy
