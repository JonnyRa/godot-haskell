...
module Godot.Tools.FileSystemDock
       (Godot.Tools.FileSystemDock.sig_display_mode_changed,
        ...
        Godot.Tools.FileSystemDock._file_sort_popup,
...
{-# NOINLINE bindFileSystemDock__file_sort_popup #-}

bindFileSystemDock__file_sort_popup :: MethodBind
bindFileSystemDock__file_sort_popup
  = unsafePerformIO $
      withCString "FileSystemDock" $
        \ clsNamePtr ->
          withCString "_file_sort_popup" $
            \ methodNamePtr ->
              godot_method_bind_get_method clsNamePtr methodNamePtr

_file_sort_popup ::
                   (FileSystemDock :< cls, Object :< cls) => cls -> Int -> IO ()
_file_sort_popup cls arg1
  = withVariantArray [toVariant arg1]
      (\ (arrPtr, len) ->
         godot_method_bind_call bindFileSystemDock__file_sort_popup
           (upcast cls)
           arrPtr
           len
           >>=
...
instance NodeMethod FileSystemDock "_file_sort_popup" '[Int]
           (IO ())
         where
        nodeMethod = Godot.Tools.FileSystemDock._file_sort_popup
